...
\begin{document}
    ...
    \maketitle
\begin{abstract}
    Le notazioni utilizzate in questo schema sono $\vec{x}$ per rappresntare un generico array x, dim indica sempre una dimensione, xCode indica un codice di root inerente a x, opt indice sempre la stringa di caratteri che ROOT interpreta come opzioni.
\end{abstract}
\section{Istogrammi}
...
\section{Fitting}
    ...
    \begin{itemize}
        ...
        \item \colorbox{LightGray}{GetParameters(par)} mette nell'array par i parametri
        \item \colorbox{LightGray}{GetParErrors(epar)}
    \end{itemize}
\section{Random}
    ...
    \begin{itemize}
        ...
        \item \colorbox{LightGray}{FillRandom("function", n)} filla l'oggetto con n elementi della function 
    \end{itemize}
\section{Canvas}
...
\section{Appendice}
    ...
    \subsection{Risoluzione o Smearing}
        ...
        \begin{verbatim}
            for(Int_t i=0;i<nGen;i++)h[1]->Fill(gRandom->Gaus(distribution,res));
        \end{verbatim}
    \subsection{Efficienza}
        ...
        Es:
        \begin{verbatim}
            TF1 *eff=new TF1(“eff”, funzione, min, max);  [Es: funzione=0.2*x]
            x=gRandom->Gaus(mean, sigma);\\
            y=gRandom->Rndm();\\
            if(y<eff->Eval(x)){h1->Fill(x)}
        \end{verbatim}
